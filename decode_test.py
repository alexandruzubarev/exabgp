...
"""
@alexandru zubarev:
Is created class Neighbor with a set of parameters, values and capabilities, that represent the data from the BGP UPDATE message.
Below is defined a static method families with following decorator @staticmethod that returns value of the known families.
"""
...
"""
@alexandru zubarev:
Is created class TestUpdateDecoding with two test cases:
1. test_decoding_uodate_asn
2. test_decoding_update_asn4
setUp is test fixture method that will run prior ro each test case.
Defined BGP OPEN message and its data that is used to open a BGP peering session.
The OPEN message contains information about the BGP neighbor initiating session and information about supported and negotiated options, like:
- BGP version number
- local as number
- BGP identifier that indicates the IP of the sender of the BGP OPEN message
- capabilities, such as neighbor families
- Hold Time that is 180 seconds.
Test case below is testing if the asn4 data is found in bodies (hexa representation of a BGP packet) then is performed the BGP message decoding/unpacking.
"""
