...
"""
@alexandru zubarev:
Test class TestL2VPN. Defined static method generate, that contains a set of parameters an is returned the RouteDistinguisher joined data.
In the setUp method nlri_1 and nlri_2 data of l2vpn packet is encoded as an array of bytes from hexa format and decoded as a decimal format.
It is initialized a new bytearray object/instance from a string of hex numbers, that represent the extended-community info from L2VPN packet.
test_l2vpn_decode and test_lsvpn_encode are just defined methods that has commented code.
But there are tests that would unpack VPLS packet and would test if the decoded/encoded data of l2vpn packets are matching the expected data.
"""
