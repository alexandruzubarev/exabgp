...
"""
@alexandru zubarev:
Defined TestData class with 2 tests: test_1_open, test_2_open.
test_1_open:
In order to use route refresh is needed firstly to enable/set the capability, as it is made.
Created message object with 3 parameters of unpacked message:id_message, body, negotiated.
It checks the capability of open message to be equal to the expected ones, otherwise test is considered invalid.
BGP include an AFI and a SAFI with every route advertisement.

test_2_open:
OPEN message is created with parameters:
- BGP version number
- ASN of the originating router
- BGP Identifier
- session capabilities params 
- Hold Time.
Assertion methods are used for checking if the capabilities are matching for the test to pass.  
"""
