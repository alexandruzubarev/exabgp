...
"""
@alexandru zubarev: 
Are declared two BGP packets: OPEN and KEEP.
Is created an outgoing connection.
To writer are sent the defined packets: OPEN and KEEP.
From reader are returned packet details, such as: size, header, msg, body, notification.
Then the connection is close.
The test scope is to check the connection read/write functional.
"""
...
"""
@alexandru zubarev:
Defined class TestData with 2 tests (with commented code below).
test_1 would execute function test on enabled profiling.
test_2 would run the profiling for function test.
"""
